<?php
/**
* Plugin Name: Formulario de Registro de Beneficiarios FMJGH (API)
* Description: Formulario de registro via API CONAPDIS
* Version: 4.0.0
...
define('BENEFICIARIO_API_URL', home_url('/php/api_conapdis.php'));
define('BENEFICIARIO_API_KEY', 'sk_sidaii_local_dev_2026');
...
class CONAPDIS_Beneficiario_Form {
	...
	public function renderizar_formulario() {
							...
							<div class="conapdis-form-grid">
								<div class="conapdis-form-group">
									...
									<select id="tipoDiscapacidad" name="tipoDiscapacidad" required>
										<option value="">Cargando discapacidades...</option>
									</select>
								</div>
								<div class="conapdis-form-group">
		...
		jQuery(document).ready(function($) {
			const API_URL = '<?php echo BENEFICIARIO_API_URL; ?>';
			const API_KEY = '<?php echo BENEFICIARIO_API_KEY; ?>';
			...
			function loadUbicaciones() {
				estadoSelect.empty().append('<option value="">Cargando estados...</option>').prop('disabled', true);
				$.ajax({ url: API_URL + '?action=ubicaciones', headers: { 'X-API-Key': API_KEY },
					success: function(res) {
						if (res.success && Array.isArray(res.data)) {
							ubicacionesData = res.data;
							estadoSelect.empty().append('<option value="">Seleccione estado</option>');
							ubicacionesData.forEach(function(e) {
								estadoSelect.append($('<option>', { value: e.nombre, text: e.nombre }));
							});
							estadoSelect.prop('disabled', false);
							municipioSelect.prop('disabled', true);
							parroquiaSelect.prop('disabled', true);
						} else {
							estadoSelect.empty().append('<option value="">Error al cargar estados</option>');
						}
					},
					error: function() {
						estadoSelect.empty().append('<option value="">Error de conexión</option>');
					}
				});
			}
			function loadMunicipios() {
				...
				const estado = ubicacionesData.find(function(e) { return e.nombre === estadoNombre; });
				if (estado && estado.municipios && estado.municipios.length) {
			...
			function loadParroquias() {
				...
				const estado = ubicacionesData.find(function(e) { return e.nombre === estadoNombre; });
				if (estado) {
					const municipio = estado.municipios.find(function(m) { return m.nombre === municipioNombre; });
					if (municipio && municipio.parroquias && municipio.parroquias.length) {
						municipio.parroquias.forEach(function(p) {
			...
			function loadDiscapacidades() {
				const discSelect = $('#tipoDiscapacidad');
				$.ajax({ url: API_URL + '?action=discapacidades', headers: { 'X-API-Key': API_KEY },
					success: function(res) {
						if (res.success && Array.isArray(res.data)) {
							discSelect.empty().append('<option value="">Seleccione tipo de discapacidad</option>');
							res.data.sort(function(a, b) { return a.nombre_e.localeCompare(b.nombre_e); });
							res.data.forEach(function(d) {
								discSelect.append($('<option>', { value: d.id_e, text: d.nombre_e }));
							});
						} else {
							discSelect.empty().append('<option value="">Error al cargar discapacidades</option>');
						}
					},
					error: function() {
						discSelect.empty().append('<option value="">Error de conexión</option>');
					}
				});
			}
			...
			function verificarCedula(cedula) {
				...
				fetch(API_URL + '?action=verificar-cedula&cedula=' + cedulaLimpia, {
				...
				.then(function(response) {
					...
					if (response.success && response.exists) {
						...
						updateCedulaStatus('invalid', 'Esta cédula ya está registrada en FMJGH');
					} else {
			...
			fechaNacimientoInput.on('change input', calcularEdad);
			...
			cedulaInput.on('input', function() {
				...
				if (limpia.length >= 6) {
					verifyTimeout = setTimeout(function() { verificarCedula(limpia); }, 500);
				} else {
					updateCedulaStatus('empty', '');
					cedulaValidation = { isValid: false, isChecking: false, isDuplicated: false };
				}
			});
			fileInputs.forEach(function(id) { $('#' + id).on('change', function() { validarArchivo(this); }); });
			...
			form.on('submit', function(e) {
				...
				if (!allFilesValid) {
					Swal.fire({ icon: 'error', title: 'Documentos incompletos', html: 'Por favor, adjunte:<br><br>' + missingFiles.map(function(f) { return '• ' + f; }).join('<br>'), confirmButtonColor: '#1e3a5f' });
					return;
				}
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						fd.append('cedula', cedulaLimpia);
						fd.append('nacionalidad', $('#nacionalidad').val());
						fd.append('email', $('#correo').val());
						fd.append('telefono', $('#telefono').val());
						fd.append('fecha_naci', $('#fechaNacimiento').val());
						fd.append('sexo', $('#sexo').val());
						fd.append('edo_civil', $('#estadoCivil').val());
						fd.append('nro_hijo', $('#numeroHijos').val() || 0);
						fd.append('estado', $('#estado').val());
						fd.append('municipio', $('#municipio').val());
						fd.append('parroquia', $('#parroquia').val());
						fd.append('direccion', $('#direccion').val());
						fd.append('discapacidad', $('#tipoDiscapacidad').val());
						fd.append('certificado', $('#numeroCertificado').val() || '');
						fd.append('proteccion_social', $('#recibeBono').val());
						// Archivos
						fileInputs.forEach(function(id) {
							var input = $('#' + id)[0];
						...
						fetch(API_URL + '?action=registrar', {
							method: 'POST',
							headers: { 'X-API-Key': API_KEY },
							body: fd
						})
						...
						.then(function(response) {
							...
							if (response.success) {
								Swal.fire({
									...
									html: response.message + '<br><br><strong>Cédula:</strong> ' + response.data.cedula,
								...
								Swal.fire({ icon: 'error', title: 'Error', text: response.message || 'Ocurrió un error', confirmButtonColor: '#1e3a5f' });
							}
						})
