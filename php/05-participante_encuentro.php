 ...
 */
		...
		public function insertarParticipante(){
			try{	
				...
				$stmt = $this->cnn->prepare("INSERT INTO participante_encuentro (cedula, fecha_recibido, encuentro) 
											VALUES (:cedula, :fecha_recibido, :encuentro)");
		...
		public function autenticar(){ 
			try{	
				...
				$stmt = $this->cnn->prepare("SELECT * FROM participante_encuentro WHERE cedula = :cedula AND passwordd = :passwordd");
		...
		public function consultarTodosParticipantes(){
			...
			try{	
				...
				$stmt = $this->cnn->prepare("SELECT participante_encuentro.id, beneficiario.nombre, beneficiario.apellido, beneficiario.cedula, participante_encuentro.fecha_recibido
				from beneficiario, participante_encuentro where beneficiario.cedula = participante_encuentro.cedula and participante_encuentro.encuentro = :encuentro");
				// Especificamos el fetch mode antes de llamar a fetch()
				$stmt->setFetchMode(PDO::FETCH_ASSOC); // Devuelve los datos en un arreglo asociativo
				$stmt->bindParam(':encuentro', $this->encuentro);
				// Ejecutamos
